...
export default function ChatWidget({
  ...
  // Lazy init: getOrCreateSession reads and writes localStorage, so it must
  // run exactly once and never during re-renders (with a session_id prop it
  // would otherwise recreate the session — wiping stored messages and
  // resetting expiry — on every render).
  ...
  // Language is resolved once at mount; there is no in-widget language switcher.
  const [currentLanguage] = useState<Language>(getInitialLanguage());
  ...
  function updateLastMessage(message: ChatMessageType) {
    setMessages((prev) =>
      prev.length === 0 ? prev : [...prev.slice(0, -1), message]
    );
  }
  ...
  // The Swarovski theme uses Euclid Circular B. @font-face has no effect
  // inside a shadow root, so it must be injected into the host document; the
  // fonts load from the repo CDN only when that theme is active instead of
  // shipping base64-embedded in the bundle for every customer.
  useEffect(() => {
    if (theme !== 'swarovski') return;

    const FONT_STYLE_ID = 'punku-euclid-font';
    if (document.getElementById(FONT_STYLE_ID)) return;

    const FONT_BASE_URL = 'https://cdn.jsdelivr.net/gh/PUNKU-AI/punku-embedded-chat/public/fonts/euclid';
    const fontStyle = document.createElement('style');
    fontStyle.id = FONT_STYLE_ID;
    fontStyle.textContent = `
@font-face {
  font-family: 'Euclid Circular B';
  src: url('${FONT_BASE_URL}/Euclid%20Circular%20B%20Light.ttf') format('truetype');
  font-weight: 300;
  font-style: normal;
  font-display: swap;
}
@font-face {
  font-family: 'Euclid Circular B';
  src: url('${FONT_BASE_URL}/Euclid%20Circular%20B%20Regular.ttf') format('truetype');
  font-weight: 400;
  font-style: normal;
  font-display: swap;
}`;
